    ...
    <div className="card">
      <div className="card-body position-relative p-lg-5 p-3">
        <div className="position-absolute end-0 me-lg-5 me-3">
          <Link to="/profile" className="btn btn-primary me-1" onClick={saveChanges}>Save</Link>
        ...
        <div className="text-muted mb-3">@{profile.id}</div>
        <div className="mb-3">
        ...
        <div className="mb-3">
          ...
          <p className="text-muted mb-0">{imagesUserMap[profile.id]?.length || '0'} Submission{imagesUserMap[profile.id]?.length !== 1 ? 's':''}</p>
        </div>
